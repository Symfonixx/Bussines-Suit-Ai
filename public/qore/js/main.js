...
(function ($) {
    ...
    var infiniteSlide = function () {
        if ($(".infiniteSlide").length > 0) {
            $(".infiniteSlide").each(function () {
                var $this = $(this);
                if ($this.parent().hasClass("infiniteslide_wrap") || $this.data("qore-inited")) {
                    return;
                }
                $this.data("qore-inited", true);
    ...
    -------------------------------------------------------------------------*/
    var activeTab = () => {
        $(".tab-can_do .btn_tab").off("click.qore").on("click.qore", function () {
    ...
    var animateBorder = () => {
        ...
        if (!$menu.length || $menu.find(".menu-indicator").length) {
            return;
        }
    ...
    const activeAccordion = () => {
        $(".faq-accordion-list .faq-accordion_item").off("click.qore").on("click.qore", function () {
            const $this = $(this);
    ...
    var pageEffects = function () {
        ...
        if (typeof WOW !== "undefined") {
            ...
            new WOW({ live: false, offset: 40 }).init();
        }
    ...
    // Dom Ready — Vue owns the mobile menu (Inertia Links).
    $(function () {
        headerSticky();
        goTop();
        activeSection();
        scrollLink();
        pageEffects();
        window.initQoreTheme = pageEffects;
    });
})(jQuery);
